...
class TestErrorCollection:
    ...
    def test_derived_field_error_is_collected(self):
        ...
        # Config key must match source name for derived fields to be applied
        config = {"items": RelationshipConfig(derived_fields=["computed = item.nonexistent.field"])}
    ...
    def test_error_includes_source_name(self):
        ...
        # Config key must match source name - test with customers
        config = {"customers": RelationshipConfig(derived_fields=["bad_field = item.missing.path"])}

        self.handler.process(data, config)

        errors = self.handler.get_errors_as_dicts()
        # Errors should include the source name
    ...
    def test_error_includes_field_and_expression(self):
        ...
        data = {"items": [{"value": 10}]}
        # Config key must match source name
        config = {"items": RelationshipConfig(derived_fields=["result_field = item.a + item.b"])}
    ...
    def test_context_addition_error_is_collected(self):
        ...
        config = {"test": RelationshipConfig(context_additions={"users": "Hello {{item.name}}"})}
    ...
    def test_clear_errors(self):
        ...
        data = {"items": [{"value": 10}]}
        # Config key must match source name
        config = {"items": RelationshipConfig(derived_fields=["x = item.y"])}
